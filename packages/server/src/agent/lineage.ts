...
export async function findForeignTaskTip(
  ...
  const revs = await exec(
    `git -C ${shellQuote(workdir)} rev-list ${shellQuote(`${baseSha}..HEAD`)}`,
  );
  if (revs.exitCode !== 0) {
    throw new Error(`lineage rev-list failed in ${workdir}: ${revs.stderr.trim()}`);
  }
  ...
  for (const candidate of candidates) {
    ...
    for (const ref of refs) {
      const foreign = await exec(
        `git -C ${shellQuote(workdir)} rev-list ${shellQuote(`${baseSha}..${ref}`)}`,
      );
