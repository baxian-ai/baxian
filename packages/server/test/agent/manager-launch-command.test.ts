...
describe('buildLaunchCommand', () => {
  it('claude-code: bypass mode by default', () => {
    expect(buildLaunchCommand(makeAgent({ runtime: 'claude-code' }))).toBe(
  ...
  it('claude-code uses env instead of POSIX assignment syntax for shell compatibility', () => {
    const cmd = buildLaunchCommand(makeAgent({ runtime: 'claude-code' }));
  ...
  it('claude-code disables the session feedback survey; codex carries no such env', () => {
    expect(buildLaunchCommand(makeAgent({ runtime: 'claude-code' }))).toContain('CLAUDE_CODE_DISABLE_FEEDBACK_SURVEY=1');
    expect(buildLaunchCommand(makeAgent({ runtime: 'codex' }))).not.toContain('CLAUDE_CODE_DISABLE_FEEDBACK_SURVEY');
  });
  ...
  it('claude-code with --model', () => {
    expect(buildLaunchCommand(makeAgent({ runtime: 'claude-code', model: 'sonnet' }))).toBe(
  ...
  it('claude-code with single --add-dir', () => {
    expect(buildLaunchCommand(makeAgent({ runtime: 'claude-code', addDirs: ['/usr/local/lib'] }))).toBe(
  ...
  it('claude-code with both --model and --add-dir', () => {
    expect(
      buildLaunchCommand(makeAgent({ runtime: 'claude-code', model: 'opus', addDirs: ['/x'] })),
  ...
  it('empty addDirs array is a no-op', () => {
    expect(buildLaunchCommand(makeAgent({ runtime: 'claude-code', addDirs: [] }))).toBe(
      'env CLAUDE_CODE_NO_FLICKER=1 CLAUDE_CODE_DISABLE_FEEDBACK_SURVEY=1 claude --permission-mode bypassPermissions',
    );
  });
  ...
  it('shell-quotes addDirs with spaces and metacharacters', () => {
    const cmd = buildLaunchCommand(
      makeAgent({ runtime: 'claude-code', addDirs: ['/path with space/$(whoami)'] }),
    );
    expect(cmd).toBe(
  ...
  it('opencode: auto-approve mode by default', () => {
    expect(buildLaunchCommand(makeAgent({ runtime: 'opencode' }))).toBe('env OPENCODE_DISABLE_CLAUDE_CODE_SKILLS=1 opencode --auto');
  });

  it('opencode with --model', () => {
    expect(buildLaunchCommand(makeAgent({ runtime: 'opencode', model: 'anthropic/claude-sonnet-4-6' }))).toBe(
  ...
  it('opencode drops addDirs: it has no --add-dir flag', () => {
    expect(buildLaunchCommand(makeAgent({ runtime: 'opencode', addDirs: ['/a', '/b'] }))).toBe('env OPENCODE_DISABLE_CLAUDE_CODE_SKILLS=1 opencode --auto');
  });

  it('qodercli: skip-permissions mode by default', () => {
    expect(buildLaunchCommand(makeAgent({ runtime: 'qodercli' }))).toBe(
      'qodercli --dangerously-skip-permissions',
    );
  });

  it('qodercli with --model and --add-dir', () => {
    expect(buildLaunchCommand(makeAgent({ runtime: 'qodercli', model: 'efficient', addDirs: ['/x'] }))).toBe(
  ...
  describe('yolo: false launches the runtime in its default permission mode', () => {
    it.each<[AgentRuntime, string]>([
      ...
      expect(buildLaunchCommand(makeAgent({ runtime, yolo: false }))).toBe(expected);
    });

    it.each<[AgentRuntime]>([['claude-code'], ['codex'], ['opencode'], ['qodercli']])(
      '%s: explicit yolo: true matches the undefined default',
      (runtime) => {
        expect(buildLaunchCommand(makeAgent({ runtime, yolo: true }))).toBe(
          buildLaunchCommand(makeAgent({ runtime })),
        );
      },
    );
    ...
    it('keeps --model and --add-dir appended without the bypass flag', () => {
      expect(
        buildLaunchCommand(makeAgent({ runtime: 'claude-code', yolo: false, model: 'opus', addDirs: ['/x'] })),
      ).toBe(
...
describe('launchCommandIn', () => {
  ...
  it('shell-quotes directories with spaces', () => {
    expect(launchCommandIn('/tmp/my repo', makeAgent({ runtime: 'claude-code' }))).toContain(
      "cd '/tmp/my repo' && ",
    );
  });
});
