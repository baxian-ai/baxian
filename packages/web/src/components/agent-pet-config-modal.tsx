  ...
  return (
    <Modal open onClose={onClose} title="Agent Pet" size="md">
      <div className="space-y-4">
        <label className="flex items-center gap-2 text-[13px] text-og-1000">
          <input
            type="checkbox"
            checked={enabled}
            disabled={busy}
        ...
        {!enabled ? (
          <p className="text-[12px] text-og-500">
          ...
          <>
            <div>
              ...
              />
              <p className="mt-1 text-[12px] text-og-400">选择 Codex Pet 目录（含 pet.json 与精灵图）。</p>
            </div>

            {loading ? (
              <p className="text-[13px] text-og-500">加载中…</p>
            ) : pets.length === 0 ? (
              <p className="text-[13px] text-og-500">还没有上传过 Pet，点击上方按钮上传。</p>
            ) : (
                  ...
                  return (
                    ...
                    >
                      ...
                      <button
                        ...
                        className="min-w-0 flex-1 truncate text-left text-[13px] text-og-1000 hover:text-accent-hover disabled:cursor-default"
                      >
                        {pet.displayName}
                        {selected && <span className="ml-1 text-[11px] text-accent">（当前）</span>}
                      </button>
                      <button
                        type="button"
                        ...
                        disabled={busy}
                        className="shrink-0 text-[12px] text-danger hover:underline disabled:opacity-50"
                      >
                        删除
                      </button>
                    </li>
