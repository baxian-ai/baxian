...
export function PendingRestartBanner() {
  ...
  if (phase === 'failed') {
    return (
      <div className="flex items-center justify-between border-b border-[#fecaca] bg-[#fef2f2] px-4 py-2">
        <div className="text-[14px] text-danger">❌ 重启失败：{error}</div>
  ...
  if (phase === 'restarting') {
    return (
      <div className="border-b border-accent-soft bg-accent-soft/40 px-4 py-2 text-[14px] text-accent">
        🔄 重启中…
      </div>
    );
  }
  ...
  return (
    <div className="flex items-center justify-between border-b border-[#fde68a] bg-[#fef3c7]/60 px-4 py-2">
      <div className="text-[14px] text-warn">⚠️ 有 {count} 项配置变更待重启 baxian server 才生效</div>
